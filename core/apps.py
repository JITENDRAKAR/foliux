...
class CoreConfig(AppConfig):
    ...
    def ready(self):
        ...
        if os.environ.get('RUN_MAIN') == 'true' or os.environ.get('SERVER_SOFTWARE', '').startswith('gunicorn'):
            ...
            scheduler.add_job(perform_sync, 'interval', minutes=10, id='gsheet_sync_job', replace_existing=True)
            ...
            scheduler.start()
            print("Background scheduler started for sync and auto updates (30m interval).")
            ...
            scheduler.start()
            print("Background scheduler started (IIS/Generic) with auto updates (30m interval).")
